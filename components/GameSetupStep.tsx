...
function getAdjustedReq(
  ...
  return { townsfolk: Math.max(0, tf), outsider: Math.max(0, os), minion: rawReq.minion, demon: rawReq.demon };
}
...
function AnalysisSidebar({
  ...
  if (gameIds.length === 0) {
    return (
      <div style={{ color: "#333", fontFamily: "var(--font-garamond)", fontSize: 13, textAlign: "center", padding: "40px 16px" }}>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      {/* Strength */}
      <div>
        <div style={{ fontFamily: "var(--font-cinzel)", fontSize: 10, color: "#b8965a", letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 10 }}>
          Team Strength
        </div>
          ...
          {[
            ...
            <div key={label}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 3 }}>
                <span style={{ fontFamily: "var(--font-jetbrains)", fontSize: 9, color, textTransform: "uppercase" }}>{label}</span>
              ...
              <div style={{ height: 6, background: "#1a1a2a", borderRadius: 3, overflow: "hidden" }}>
                <div style={{ height: "100%", width: `${(Math.abs(value) / maxAbs) * 100}%`, background: color, borderRadius: 3 }} />
              </div>
            </div>
          ))}
        </div>
              ...
              return (
                <div key={key}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 2 }}>
                    <span style={{ fontFamily: "var(--font-jetbrains)", fontSize: 9, color: "#555", textTransform: "uppercase" }}>{label}</span>
                    <span style={{ fontFamily: "var(--font-cinzel)", fontSize: 9, color }}>{val}</span>
                  </div>
                  <div style={{ display: "flex", gap: 2 }}>
                    {levels.map((_, i) => (
                      <div key={i} style={{ flex: 1, height: 5, borderRadius: 2, background: i <= idx ? color : "#2a2a3a" }} />
                    ))}
                  </div>
                </div>
              );
      ...
      {keyHints.length > 0 && (
        <div>
          <div style={{ fontFamily: "var(--font-cinzel)", fontSize: 10, color: "#b8965a", letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 8 }}>
              ...
              return (
                ...
                >
                  <div style={{ display: "flex", alignItems: "center", gap: 5, marginBottom: 4, flexWrap: "wrap" }}>
                    {isJinx && (
                      <span style={{ fontFamily: "var(--font-cinzel)", fontSize: 8, color: "#b8965a", background: "#2a1f00", border: "1px solid #7a6200", borderRadius: 3, padding: "1px 4px", flexShrink: 0 }}>
                        ⚖ Jinx
                      </span>
                    )}
                    {involvedChars.map((c) => (
                      <span key={c.id} style={{ fontFamily: "var(--font-cinzel)", fontSize: 10, color: "#b8965a", background: "#2a2a3a", borderRadius: 3, padding: "1px 5px" }}>
                        {c.name}
                      </span>
                    ))}
                  </div>
                  <div style={{ fontFamily: "var(--font-cinzel)", fontSize: 11, color: "#e8dcc8", marginBottom: 3 }}>
                    {hint.title}
                  </div>
                  <div style={{ fontFamily: "var(--font-garamond)", fontSize: 12, color: "#c8b89a", lineHeight: 1.5 }}>
                    {hint.description}
                  </div>
                </div>
              );
              ...
              .map((entry) => {
                ...
                return (
                  <div key={char.id} style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    <div style={{ fontFamily: "var(--font-cinzel)", fontSize: 10, color: col.text, minWidth: 90, flexShrink: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                      {char.name}
                    </div>
                    <div style={{ flex: 1, height: 4, background: "#1a1a2a", borderRadius: 2, overflow: "hidden" }}>
                      <div style={{ height: "100%", width: `${(Math.abs(s) / 100) * 100}%`, background: barColor, borderRadius: 2 }} />
                    </div>
                    <div style={{ fontFamily: "var(--font-jetbrains)", fontSize: 9, color: barColor, minWidth: 28, textAlign: "right", flexShrink: 0 }}>
                      {s > 0 ? "+" : ""}{s}
                    </div>
                    {eff.modifier !== 0 && (
                      <div style={{ fontFamily: "var(--font-jetbrains)", fontSize: 8, color: eff.modifier > 0 ? "#2a7fd4" : "#c0392b", flexShrink: 0 }}>
...
export function GameSetupStep({
  ...
  const gameCounts = {
    townsfolk: gameIds.filter((id) => allCharacters.find((c) => c.id === id)?.team === "townsfolk").length,
    outsider:  gameIds.filter((id) => allCharacters.find((c) => c.id === id)?.team === "outsider").length,
    minion:    gameIds.filter((id) => allCharacters.find((c) => c.id === id)?.team === "minion").length,
    ...
    >
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
        ...
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
          <div>
            <h2 style={{ fontFamily: "var(--font-cinzel)", fontSize: 22, color: "#e8dcc8", margin: "0 0 6px", letterSpacing: "0.04em" }}>
              Step 2 — Set Up Your Game
            </h2>
            <div style={{ fontFamily: "var(--font-garamond)", fontSize: 13, color: "#555" }}>
              Script: <span style={{ color: "#b8965a" }}>{EDITION_LABELS[scriptSource ?? "custom"]}</span>
              {" · "}{scriptIds.length} characters available
            </div>
          </div>
          <button
            onClick={onBack}
            style={{ background: "none", border: "1px solid #2a2a3a", borderRadius: 5, padding: "6px 14px", color: "#666", cursor: "pointer", fontFamily: "var(--font-garamond)", fontSize: 13 }}
          >
            ← Change Script
          </button>
        </div>
        ...
        <div>
            ...
            {Array.from({ length: 16 }, (_, i) => i + 5).map((n) => {
              ...
              return (
                ...
                >
                  {n}
                  {isTraveler && (
                    <span style={{ fontSize: 8, color: active ? "#e8dcc8" : "#444", display: "block", lineHeight: 1 }}>
                      +{n - 15}T
                    </span>
                  )}
                </button>
          ...
          {travelerCount > 0 && playerCount && (
            <div style={{ fontFamily: "var(--font-garamond)", fontSize: 12, color: "#555", marginTop: 8 }}>
              Travelers fill the extra {travelerCount} slot{travelerCount > 1 ? "s" : ""} — they are not from your script.
            </div>
          )}
        </div>
        ...
        {playerCount && req && (
          <>
            {/* Distribution */}
            <div style={{ background: "var(--bg-surface)", border: "1px solid #2a2a3a", borderRadius: 10, padding: "14px 16px" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
                <div style={{ fontFamily: "var(--font-cinzel)", fontSize: 12, color: "#b8965a", letterSpacing: "0.06em", textTransform: "uppercase" }}>
                  {playerCount}-Player Distribution
                </div>
                {isComplete && (
                  <div style={{ fontFamily: "var(--font-cinzel)", fontSize: 11, color: "#2d6a4f" }}>✓ Roster Complete</div>
                )}
              </div>
              <div style={{ display: "flex", gap: 8 }}>
                {TEAM_ORDER.map((team) => {
                  ...
                  return (
                    <div key={team} style={{ flex: 1, background: "#0a0a14", border: `2px solid ${done ? c.border : "#2a2a3a"}`, borderRadius: 8, padding: "10px 6px", textAlign: "center", transition: "border-color 0.2s" }}>
                      <div style={{ fontFamily: "var(--font-jetbrains)", fontSize: 22, color: done ? c.text : "#333", lineHeight: 1 }}>{have}</div>
                      <div style={{ fontFamily: "var(--font-jetbrains)", fontSize: 10, color: "#333", margin: "2px 0" }}>/ {needed}</div>
                      <div style={{ fontFamily: "var(--font-garamond)", fontSize: 11, color: "#555", textTransform: "capitalize" }}>{team}</div>
                    </div>
                  );
                })}
              </div>
              {activeModifiers.length > 0 && (
                <div style={{ marginTop: 10, display: "flex", flexDirection: "column", gap: 3 }}>
                  {activeModifiers.map((note, i) => (
                    <div key={i} style={{ fontFamily: "var(--font-garamond)", fontSize: 12, color: "#d5825b" }}>⚙ {note}</div>
                  ))}
                </div>
              )}
            </div>
            ...
            {/* Character selection */}
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              <div style={{ fontFamily: "var(--font-cinzel)", fontSize: 11, color: "#b8965a", letterSpacing: "0.08em", textTransform: "uppercase" }}>
                ...
                return (
                  <div key={team}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
                      <div style={{ fontFamily: "var(--font-cinzel)", fontSize: 12, color: c.text, letterSpacing: "0.06em", textTransform: "uppercase" }}>
                        {TEAM_LABEL[team]}
                      </div>
                      <div style={{ fontFamily: "var(--font-jetbrains)", fontSize: 11, color: isFull ? c.text : "#555", background: "#0a0a14", border: `1px solid ${isFull ? c.border : "#2a2a3a"}`, borderRadius: 10, padding: "2px 10px" }}>
                        {have} / {needed}
                      </div>
                      {needed === 0 && (
                      ...
                      {chars.map((char) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <button
                              onClick={() => { if (!blocked) onToggleGameChar(char.id); }}
                              disabled={blocked}
                              style={{
                                background: inGame ? c.bg : "#14141f",
                                border: "none",
        ...
        {!playerCount && (
          <div style={{ color: "#444", fontFamily: "var(--font-garamond)", fontSize: 14, textAlign: "center", padding: "30px 0" }}>
      ...
      >
        <div style={{ fontFamily: "var(--font-cinzel)", fontSize: 10, color: "#b8965a", letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 12 }}>
          Live Analysis
        </div>
